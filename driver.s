...
// Date last modified: 4/9/2024
 ...
 .global _start
  ...
  szStar1:         .asciz "hat."        // holds test value
  szStar2:         .asciz "Cat"         // holds test value 
  szEnd:           .asciz "in the hat."         // holds test value
...
/******* STRING LENGTH *****/
 ...
 // ***** Equals (Case 1) ******/                                                                               
 LDR X0,=szOutEquals1  // load value
 BL  putstring          // print
 LDR X0,=szS1           // load x0
 LDR X1,=szS3           // load x3
 BL  Equals             // branch link equals

// ***** Equals (Case 2) ******/                                                                               
 LDR X0,=szOutEquals2           // load value
 BL  putstring                  // print
 LDR X0,=szS1                   //load x0
 LDR X1,=szS1                   // load x1
 BL  Equals             // brnach link equals

// ***** IgnoreCase (Case 1) ******/                                                                            
 LDR X0,=szOutIgnoreCase1 // load value
 BL  putstring  // print
 LDR X0,=szS1  // load register
 LDR X1,=szS3  // load register
 BL  IgnoreCase  // branch link ignore case

// ***** IgnoreCase (Case 2) ******/                                                                            
 LDR X0,=szOutIgnoreCase2  // load register
 BL  putstring  // print
 LDR X0,=szS1  // load register
 LDR X1,=szS2  // load regiser
 BL  IgnoreCase  // branch link ignorecase

// ***** strCopy *****/
  LDR   X0,=szCopy // load register
  BL    putstring // print
  LDR X0,=szOutCopy // load register
  BL  putstring // print
  LDR X0,=chLF // load register
  BL  putch // print
  LDR X0,=szCopied // load register
  BL  putstring // print
  LDR X0,=szS1  // load register
  BL  putstring // print
  LDR   X0,=chLF // load register
  ...
  BL    putstring  // print
  LDR X0,=szS1  // load register
  BL  copy  // link to copy
  BL  putstring   // print
  LDR X0,=chLF  // load register
  BL  putch  // line break

// ***** charAt    ******/                                                                       
 LDR X0,=szCharAt  // load intro
 BL  putstring  // print
 MOV W4,#4  // set parameter
 LDR X0,=szS2  // load register
 BL  charAt  // execute charAt

// **** substring_1 ****/
  LDR X0,=szOutSub1  // load into
  BL  putstring  // print
  MOV W4,#4  // set parameter
  MOV W5,#14  // set paramteter
  LDR X0,=szS3  // set parameter
  BL  substring_1  // execute fucntion

  // **** substring_2 ****/
  LDR X0,=szOutSub2 // load intro
  BL  putstring  // print 
  MOV W4,#7   // set parameter
  LDR X0,=szS3  // set parameter
  BL  substring_2  // execute funciton

  // **** startsWith_1 **** //
  LDR X0,=szOutStart1  // load intro
  BL  putstring   // print
  MOV X4,#11   // set parameter
  LDR X0,=szS1  // load parameter
  LDR X1,=szStar1  // load parameter
  BL  startsWith_1  // execute function

  // **** startsWith_2 **** //
  LDR X0,=szOutStart2 // load intro
  BL  putstring  // print intro
  LDR X0,=szS1  // load parameter
  LDR X1,=szStar2  // load parameter
  BL  startsWith_2  // execute function

  // **** endsWith **** //
  LDR   X0,=szOutEnd  // load itnro
  BL    putstring  // print
  LDR X0,=szS1  // load parameter
  LDR X1,=szEnd  // load parameter
  BL  endsWith  // execute function

  CMP X0,#0  // compare output
  BEQ false  // if equal, jump to false

  // Print true
  LDR   X0,=string_true  // load string
  BL    putstring  // print
  LDR   X0,=chLF  // laod
  BL    putch  // line break
 
  B     jump_over  // skip false

  false:
  LDR X0,=string_false  //load string
  BL    putstring  // print
  LDR X0,=chLF  // load 
  BL    putch  // print
